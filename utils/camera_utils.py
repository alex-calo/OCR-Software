...
def list_cameras(max_cams: int = MAX_CAMERAS) -> List[int]:
    ...
    for i in range(max_cams):
        ...
        try:
            # Windows-specific backend priority
            if platform.system() == 'Windows':
                # Try MSMF first (more stable), then DSHOW, then ANY
                backends_to_try = ['MSMF', 'DSHOW', 'ANY']
            else:
                backends_to_try = ['ANY']
...
def open_camera(index: int, width: int, height: int) -> Optional[cv2.VideoCapture]:
    ...
    # Windows-specific backend priority (MSMF first as it's more stable)
    if platform.system() == 'Windows':
        backends_to_try = ['MSMF', 'DSHOW', 'ANY']
    else:
        backends_to_try = ['ANY']
