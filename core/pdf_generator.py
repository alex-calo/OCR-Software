...
class PDFSaveThread:
    """
    Optional threaded PDF writer for background saving.
    """

    def __init__(self, pdf_generator, content, output_path):
    ...
    def run(self):
        try:
            self.pdf_generator.generate_pdf(self.content, self.output_path)
        except Exception as e:
            logger.error(f"PDFSaveThread error: {e}")
